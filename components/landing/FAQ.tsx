...
const categories = [
  ...
  },
  {
    ...
    faqs: [
      ...
      {
        q: "Is e-loan available in my state?",
        a: "We currently lend in 12 states with more launching every quarter. You'll see eligibility instantly when you start your application.",
      },
    ],
  },
  {
              ...
              <Accordion type="single" collapsible className="space-y-3">
                {cat.faqs.map((faq, i) => (
                  <AccordionItem
                    key={i}
                    value={`${cat.id}-${i}`}
                    className="rounded-2xl border border-border bg-card px-5 shadow-soft data-[state=open]:shadow-card"
                  >
